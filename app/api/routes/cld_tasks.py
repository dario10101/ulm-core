...
def list_cld_tasks(
    ...
    if date_ is not None:
        return service.list_for_range(user_id, date_, date_, tz)
    if first_day is not None and last_day is not None:
        return service.list_for_range(user_id, first_day, last_day, tz)
...
def enable_cld_task_checklist_sync(
    ...
    try:
        task, added = service.enable_checklist_sync(user_id, task_id, occurrence_date, tz)
    except CldTaskNotFoundError:
        raise HTTPException(status_code=404, detail="Tarea no encontrada")
    return CldTaskChecklistSyncRead(task=task, added_to_current_week=added)
