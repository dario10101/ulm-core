...
from app.api.deps import get_cld_task_service
...
def _cld_task_to_read(task: CldTask) -> CldTaskRead:
    return CldTaskRead(
        id=task.id,
        name=task.name,
...
def list_cld_tasks(
    ...
    if date_ is not None:
        occurrences = service.list_for_range(settings.default_user_id, date_, date_)
    elif first_day is not None and last_day is not None:
        occurrences = service.list_for_range(settings.default_user_id, first_day, last_day)
    else:
    ...
    return [
        ...
        for task, occurrence_at in occurrences
    ]


@router.post("", response_model=CldTaskRead, status_code=201)
def create_cld_task(
    payload: CldTaskCreate, service: CldTaskService = Depends(get_cld_task_service)
) -> CldTaskRead:
    try:
...
def delete_cld_task(
    ...
    try:
        service.delete_task(settings.default_user_id, task_id, occurrence_date)
    except CldTaskNotFoundError:
        raise HTTPException(status_code=404, detail="Tarea no encontrada")
...
def enable_cld_task_checklist_sync(
    ...
    try:
        task, added = service.enable_checklist_sync(settings.default_user_id, task_id, occurrence_date)
    except CldTaskNotFoundError:
        raise HTTPException(status_code=404, detail="Tarea no encontrada")
    return CldTaskChecklistSyncRead(task=_cld_task_to_read(task), added_to_current_week=added)
